...
def fall_detection_func(lock,detection_lock, csi_amplitude_array,csi_shape,detection_data_array, threshold=None ,start_len=1000 ,detection_gap=1 ,alarm_interval=3000, chosen_subcarrier=20, method=1,cache_len=100):
    set_threshold = False
    count=0
    alarm_count=0
                ...
                if set_threshold and count < start_len:
                    ...
                    if count % detection_gap == 0:
                        ...
                        if alarm_count == 0 and cos <= threshold.value:
                            message = "Fall detected at " + str(datetime.now())
                            print(message)
                            alarm_count += 1
                        if alarm_count != 0 :
